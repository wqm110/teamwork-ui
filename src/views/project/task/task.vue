<template>
    <div class="task-info">
        <div class="project-navigation">
            ...
            <section class="nav-body">
                <ul class="nav-wrapper nav nav-underscore pull-left">
                    ...
                    <li class=""><a class="app" data-app="build" @click="goPage('/project/build/' + project_id)"> 版本 </a>
                    </li>
                </ul>
            </section>
            <div class="project-nav-footer">
                <a class="footer-item" :class="{'active':user_menu_modal}" @click="user_menu_modal = !user_menu_modal,menu_modal = false">
                    <Icon type="android-people"></Icon>
                    <span>{{ project_user_list.length }}</span>
                </a>
                <a class="footer-item" :class="{'active':menu_modal}" @click="menu_modal = !menu_modal,user_menu_modal = false">
        ...
        <wrapper-content>
            <ul class="board-scrum-stages">
                <li class="scrum-stage" :class="{ 'fixed-creator': task_type.fixed_creator == true}"
                    v-for="(task_type,index) in task_type_list">
                                ...
                                <li class="task task-card ui-sortable-handle"
                                    ...
                                    <div class="task-content-set open-detail">
                                        <div class="task-content-wrapper">
                                            ...
                                            <img v-if="task.executor_user_info.avatar" :src="task.executor_user_info.avatar" :title="task.executor_user_info.realname"
                                ...
                                <form class="task-creator">
                                    ...
                                    <div class="handler-wrap">
                                        <Dropdown trigger="click" @on-click="selectExecutor">
                                            ...
                                            <DropdownMenu slot="list">
                                                <DropdownItem v-if="project_user_list.length > 0" v-for="(user,index) in project_user_list" :key="index"
                                                              :name="index">
                                                    <img class="avatar img-circle img-24 pull-left m-r-sm "
                                                         :src="user.avatar">
                                                    <span class="muted"
                                                          style="line-height: 25px;">{{ user.realname }}</span>
                                                    <Icon type="ios-checkmark-empty"  size="24" style="float: right" class="muted" v-if="user.id == default_executor.id"></Icon>
                                                </DropdownItem>
                                            </DropdownMenu>
                                        </Dropdown>
                                    </div>
                                ...
                                <div class="activity-card activity-task">
                                    ...
                                    <div class="activity-content">
                                        <span class="activity-card-title">
                                            <b>{{ log.user_name }}</b> <span class="action">{{  log.ticket }} {{ log.content }}</span><span v-if="log.log_type != 'upload' && log.task_title != null">：{{ log.task_title }}</span>
                                        </span>
                        ...
                        <a class="menu-user-list">
                            ...
                            <div class="user-info">
                                <div class="user-item">
                                    ...
                                    <a class="pull-right muted" v-if="user.is_owner == 0" @click="del_user_id = user.u_user_id,del_project_user_modal = !del_project_user_modal">移出</a>
                                </div>
        ...
        <project-setting :showModal="project_setting_modal" :project_id="project_id" @on-modal-change="modalProjectSettingChange" @on-update="projectUpdate">
...
//          this.user_menu_modal = true
        }
      },
      del_project_user_modal (value){
//          this.user_menu_modal = !value
      },
    ...
    created() {
      ...
      app.$options.sockets.task_dynamic = (data) => {
        ...
        if(notice_data.user_id != app.user_info.id && notice_data.data.project_id == app.$route.params.project_id){
          app.getTaskType()
        }
      }
      app.$options.sockets.action_synchronize = (data) => {
        const push_data = utils.getPushData(data)
        if(push_data.data.project_id == app.$route.params.project_id){
          app.getTaskType()
        }
        console.log('task_action_synchronize')
      }
    },
    mounted() {
      const app = this
    ...
    methods: {
      ...
      getProjectInfo() {
        ...
        utils.sendAjax({
          ...
          data: {
            project_id: app.$route.params.project_id
          },
          success: function (res) {
      ...
      getProjectUserList() {
        ...
        utils.sendAjax({
          ...
          data: {
            project_id: app.$route.params.project_id,
            keyword: app.project_user_keyword
          },
          success: function (res) {
...
//          this.getTaskType()
      ...
      getProjectLogList() {
        let app = this
        utils.sendAjax({
          url: 'Project_Project.getProjectLogList',
          data: {
            project_id: app.$route.params.project_id,
            contain_task_log: 1
          },
      ...
      getTaskForUserOverdue() {
        ...
        utils.sendAjax({
          ...
          success: function (res) {
            app.project_task_overdue = res.data.count
          }
        });
      },
      user_search: _.debounce(
        function () {
          // if(this.project_user_add_list.length > 0){
            this.getProjectUserAdd()
          // }
        },
        1000
      ...
      hasScrolled() {
        if (!this.timer || true) {
          let app = this
          $.each(app.task_type_list, function (k, v) {
            let scrolled = utils.hasScrolled(document.getElementById(v.key + "-section"))
      ...
      goFilePage() {
        ...
        const url = '/projectinfo/' + project_id + '/files'
